        ...
        <ul className='grid grid-cols-1 gap-4 text-gray-500 font-extrabold lg:grid-cols-2'>
          {projects.map((project) => (
            <li key={project.id} className='bg-[#fcfaf2]/30 h-[480px] w-[100%] p-4 rounded-lg'>
              <h3 className='m-2 text-xl'>{ project.title }</h3>
            <div className='grid grid-cols-1 md:grid-cols-3 lg:grid-cols-1 '>
                <div className='mx-auto my-4 w-[90%] bg-white/50 h-[240px] text-center py-4 md:col-span-2 lg:col-span-1'>
                  <a href={project.url} target='_blank' className='m-auto text-xl'>{ project.alt }</a>
                </div>
